...
function validate_register_form() {
    ...
    var email = document.forms["register_form"]["email"].value; // get email
    ...
    if (email == '' || email == null) {
        ...
        document.getElementById('register_error').innerHTML = "<p><strong>E-Mail</strong> Required.</p>";
         $("#register_error").animate({opacity: 1, top: '-50px'}, 1000);
        setTimeout(function(){
            $("#register_error").animate({opacity:0, top: '-40px'}, 1000);
        }, 3000);
    ...
    if (atpos<1 || dotpos<atpos+2 || dotpos+2 >= email.length) {
        document.getElementById('register_error').innerHTML = "<p>Invalid <strong>E-Mail</strong>.</p>";
         $("#register_error").animate({opacity: 1, top: '-50px'}, 1000);
        setTimeout(function(){
            $("#register_error").animate({opacity:0, top: '-40px'}, 1000);
        }, 3000);
    ...
    $('#register_form').ajaxForm({
        ...
        success: function(response){
            ...
            if(response == "duplicate_email") {
                document.getElementById('register_error').innerHTML = "<p><strong>E-Mail</strong> already Registered.</p>";
                $("#register_error").animate({opacity: 1, top: '-50px'}, 1000);
                setTimeout(function(){
                    $("#register_error").animate({opacity:0, top: '-40px'}, 1000);
                }, 3000);
                document.forms["register_form"]["email"].focus();
            } else if(response == "duplicate_phone") {
