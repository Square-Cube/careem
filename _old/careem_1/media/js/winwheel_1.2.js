...
var wheelImageName   = "../../media/img/prizewheel.png";	// File name of the image for the wheel.
...
prizes[0] = {"name" : "50% OFF YOUR RIDE",			"startAngle" : 0,    "endAngle" : 23};  
prizes[1] = {"name" : "FIFA VIDEO GAME 2018",		"startAngle" : 28,   "endAngle" : 74};
prizes[2] = {"name" : "FREE RIDE",					"startAngle" : 79,   "endAngle" : 125};
prizes[3] = {"name" : "50% OFF YOUR RIDE",			"startAngle" : 131,  "endAngle" : 180};
prizes[4] = {"name" : "70% OFF YOUR RIDE",			"startAngle" : 185,  "endAngle" : 235};
prizes[5] = {"name" : "FOOTBALL JERSEY",			"startAngle" : 240,  "endAngle" : 285};
prizes[6] = {"name" : "100 EGP OFF YOUR CHEQUE",	"startAngle" : 290,  "endAngle" : 333};
prizes[7] = {"name" : "50% OFF YOUR RIDE",			"startAngle" : 340,  "endAngle" : 360};

// 50% OFF YOUR RIDE -> 0 to 23 & 131 to 180 & 340 to 360
// FIFA VIDEO GAME 2018 -> 28 to 74
// FREE RIDE -> 79 to 125
// 70% OFF YOUR RIDE-> 185 to 235
// FOOTBALL JERSEY -> 240 to 285
// 100 EGP OFF YOUR CHEQUE -> 290 to 333
...
{		
	...
	if (spinMode == "random")
	{	
		var prize = $('#prize').attr('attr');
		// In this mode where the wheel stops is to be random, so get a random whole number between 0 and 360 degrees.

		if(prize == "50% OFF YOUR RIDE") {
			var stopAngles = [Math.floor(Math.random() * (360 - 340 + 1)) + 340, Math.floor(Math.random() * (180 - 131 + 1)) + 131, Math.floor(Math.random() * (23 - 0 + 1)) + 0];
			stopAngle = stopAngles[Math.floor(Math.random() * stopAngles.length)];
		} else if(prize == "FIFA VIDEO GAME 2018") {
			stopAngle = Math.floor(Math.random() * (74 - 28 + 1)) + 28;
		} else if(prize == "FREE RIDE") {
			stopAngle = Math.floor(Math.random() * (125 - 79 + 1)) + 79;
		} else if(prize == "70% OFF YOUR RIDE") {
			stopAngle = Math.floor(Math.random() * (235 - 185 + 1)) + 185;
		} else if(prize == "FOOTBALL JERSEY") {
			stopAngle = Math.floor(Math.random() * (285 - 240 + 1)) + 240;
		} else if(prize == "100 EGP OFF YOUR CHEQUE") {
			stopAngle = Math.floor(Math.random() * (333 - 290 + 1)) + 290;
		} else {
			stopAngle = 140;
		}
...
function rotateSpin(){
  // setTimeout(function(){$('.spinBtn').animate({transform: "rotate(-8deg)"}, 800,'linear');},100);
  // setTimeout(function(){$('.spinBtn').animate({transform: "rotate(8deg)"}, 800,'linear');},100);
}

function doSpin() 
{	
	$('.spinBtn').removeClass('spinRotate');
	interval = setInterval(rotateSpin, 200);
	...
	// {		
	// 	if (currentAngle == prizes[x]['endAngle'] ) {
	// 		console.log(currentAngle + " = " +  prizes[x]['endAngle']);			
	// 	};
	// }
